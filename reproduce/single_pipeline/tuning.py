...
def search_dir(args, metric):
    return osp.join(
        ...
        (
            ...
            f"_sct{getattr(args, 'top_structure_combine_train', -1)}"
            f"_ht{getattr(args, 'top_hybrid_train', -1)}"
            f"_hsf{int(bool(getattr(args, 'hybrid_include_structure_features', False)))}"
...
def default_struct_args(base_args, a_dir, c_dir, b_params, eval_test=False):
    return clone_args(
        ...
        top_structure_combine_train=getattr(base_args, "top_structure_combine_train", -1),
        ignore_val=bool(getattr(base_args, "ignore_val_structure_combine", getattr(base_args, "ignore_val", False))),
...
def default_hybrid_args(base_args, struct_dir, struct_combo_key, time_dir, eval_test=False):
    args = clone_args(
        ...
        top_k_time=1,
        train_topk=getattr(base_args, "train_topk", 100),
        top_hybrid_train=getattr(base_args, "top_hybrid_train", 200),
        hybrid_include_structure_features=bool(getattr(base_args, "hybrid_include_structure_features", False)),
