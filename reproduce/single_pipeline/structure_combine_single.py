...
EPS = 1e-12
LGBM_FIT_PROTOCOL = "test_metric_tuning_v2"
...
class BTimeline:
    def __init__(self, num_rels, num_nodes):
        ...
        self.counts_ro = np.zeros((self.num_rels, self.num_nodes), dtype=np.float32)
...
class FeatureBuilder:
    ...
    def make(self, batch_data, cand_ids, valid, scores, b_counts, history, stat_scores=None, stat_valid=None, stat_cols=None):
        sources = batch_data[:, 0].astype(np.int64)
        rels = batch_data[:, 1].astype(np.int64)
        width = cand_ids.shape[1]
        stat_scores = scores if stat_scores is None else stat_scores
        stat_valid = valid if stat_valid is None else stat_valid

        features = []
        for prefix in ("base", "a", "b", "c"):
            sc = np.where(valid, scores[prefix], 0.0).astype(np.float32, copy=False)
            full_sc = np.where(stat_valid, stat_scores[prefix], 0.0).astype(np.float32, copy=False)
            full_ranks = dense_rank(full_sc, stat_valid)
            full_max_norm = max_norm(full_sc, stat_valid)
            full_z = zscore_by_query(full_sc, stat_valid)
            if stat_cols is None:
                ranks = full_ranks
                norm = full_max_norm
                z = full_z
            else:
                safe_cols = np.where(stat_cols >= 0, stat_cols, 0)
                rows = np.arange(width, dtype=np.int64)[None, :]
                batch_rows = np.arange(cand_ids.shape[0], dtype=np.int64)[:, None]
                ranks = full_ranks[batch_rows, safe_cols]
                norm = full_max_norm[batch_rows, safe_cols]
                z = full_z[batch_rows, safe_cols]
                ranks = np.where(valid, ranks, full_ranks.shape[1] + 1)
                norm = np.where(valid, norm, 0.0).astype(np.float32, copy=False)
...
def effective_train_topk(args):
    value = int(getattr(args, "top_structure_combine_train", -1))
    if value == -1:
        return int(getattr(args, "train_topk", 100))
    return value
...
def make_out_dir(args):
    ...
    parts = [
        ...
        f"top{args.top_a}-{args.top_b}-{args.top_c}"
        f"_traink{effective_train_topk(args)}_evalfull"
        f"_iv{int(bool(getattr(args, 'ignore_val', False)))}",
        f"b-{b_hash}",
        f"lgbm-{lgbm_hash}",
        f"cfg-{cfg_hash}",
    ]
...
def selected_feature_rows(feature_builder, batch_data, cand_ids, valid, scores, b_counts, history, selected):
    lens = selected.sum(axis=1).astype(np.int32, copy=False)
    if not np.any(lens > 1):
        return [], [], []
    ...
    sel_scores = {key: np.zeros((bsz, width), dtype=np.float32) for key in scores}
...
def iter_score_blocks(a_run, c_run, b_cfg, data, split, args):
    ...
    for events, _, t_orig in snapshots:
        ...
        for batch_data, neg_arr, neg_mask in batches:
            ...
            yield batch_data, valid, base_ranks, base_loose, base_strict, base_avg, cand_ids, scores, b_counts, history
            row_offset = end
...
def iter_feature_blocks(a_run, c_run, b_cfg, data, split, feature_builder, args):
    iterator = iter_score_blocks(a_run, c_run, b_cfg, data, split, args)
    for batch_data, valid, base_ranks, base_loose, base_strict, base_avg, cand_ids, scores, b_counts, history in iterator:
...
def build_ranker_matrix(a_run, c_run, b_cfg, data, feature_builder, split, topk, args):
    ...
    preselect_before_features = int(getattr(args, "top_structure_combine_train", -1)) != -1
    if preselect_before_features:
        iterator = iter_score_blocks(a_run, c_run, b_cfg, data, split, args)
        for (
            batch_data,
            valid,
            ...
            b_counts,
            history,
        ) in tqdm(iterator, desc=f"build_{split}", leave=False):
            selected = selected_for_training(base_ranks, valid, topk)
            block_X, block_y, block_groups = selected_feature_rows(
                feature_builder,
                batch_data,
                cand_ids,
                valid,
            ...
            X_parts.extend(block_X)
            y_parts.extend(block_y)
            groups.extend(block_groups)
            num_queries += len(block_groups)
            num_rows += int(sum(block_groups))
    else:
...
def tune_lgbm(a_run, c_run, b_cfg, data, feature_builder, X_train, y_train, group, args):
    ...
    if n_trials <= 0:
        ...
        val_metrics = None
        score = None
        if not bool(getattr(args, "ignore_val", False)):
        ...
        if val_metrics is None:
            print(f"[combine] fixed LGBM trained without val eval best_iteration={best_iteration} params={params}", flush=True)
        else:
            print(
    ...
    best = {"score": -float("inf"), "model": None, "params": None, "test_metrics": None, "val_metrics": None}
    direction = "maximize"
    ...
    def objective(trial):
        ...
        test_metrics = evaluate_model(a_run, c_run, b_cfg, data, feature_builder, model, "test", args)
        ...
        print(
            f"[combine] trial={trial.number} test {format_metrics(test_metrics)} "
            f"score={score:.5f} params={params}",
            flush=True,
        )
        if score > best["score"]:
    ...
    if not bool(getattr(args, "ignore_val", False)):
        best["val_metrics"] = evaluate_model(a_run, c_run, b_cfg, data, feature_builder, best["model"], "val", args)
        print(f"[combine] best LGBM validation {format_metrics(best['val_metrics'])}", flush=True)
    print(
...
def validate_args(args):
    ...
    if int(getattr(args, "top_structure_combine_train", -1)) == 0 or int(getattr(args, "top_structure_combine_train", -1)) < -1:
        raise ValueError("--top_structure_combine_train must be -1 or positive")
...
def run_search(args):
    ...
    if bool(getattr(args, "ignore_val", False)):
        b_metrics = {}
        b_score = 0.0
        print("[combine] ignore_val=1: skipping B-only validation metric", flush=True)
    else:
        b_metrics = evaluate_b_only(data, b_cfg, args)
        b_score = metric_value(b_metrics, args.component_metric)
    b_records = [{"config": b_cfg, "metrics": b_metrics, "score": b_score}]
    print(
    ...
    for idx, (a_run, c_run, b_rec) in enumerate(combos, start=1):
        ...
        if cached is not None:
            ...
            if cached is not None:
                ...
                if record.get("val_metrics"):
                    print(f"[combine] val {format_metrics(record['val_metrics'])}", flush=True)
                elif record.get("lgbm_tuning", {}).get("test_metrics"):
                    print(f"[combine] selected by test {format_metrics(record['lgbm_tuning']['test_metrics'])}", flush=True)
                else:
                    print("[combine] cached validation metric skipped", flush=True)
        ...
        val_info = {
            "mode": "skipped" if bool(getattr(args, "ignore_val", False)) else "full_metric_stream",
            "queries": 0 if bool(getattr(args, "ignore_val", False)) else split_query_count(data, "val"),
            "eval_batch_size": int(getattr(args, "eval_batch_size", getattr(args, "block_size", 256))),
        }
        if bool(getattr(args, "ignore_val", False)):
            print("[combine] ignore_val=1: skipping validation metric stream", flush=True)
        else:
            print(
        ...
        val_metrics = lgbm_tuning.get("val_metrics")
        test_metrics_from_tuning = lgbm_tuning.get("test_metrics")
        selection_split = lgbm_tuning.get("selection_split", "val" if val_metrics is not None else "none")
        if val_metrics is not None:
            val_score = metric_value(val_metrics, args.metric)
        elif test_metrics_from_tuning is not None:
            val_score = metric_value(test_metrics_from_tuning, args.metric)
        else:
            val_score = 0.0
        record = {
        ...
        if val_metrics is not None:
            ...
            print(f"[combine] selected by test {format_metrics(test_metrics_from_tuning)}", flush=True)
        else:
            print("[combine] LGBM trained; validation metric skipped", flush=True)
    ...
    print(
        f"\n[combine] best {best_record.get('selection_split', 'val')} "
        f"{ranking_metric_key(args.metric)}={best_record['val_score']:.5f}",
        flush=True,
    )
