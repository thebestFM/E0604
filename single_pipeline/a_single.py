...
@njit(cache=True)
def _score_group_candidates(keys, values, rows, pos_obj, neg_samples, pos_out, neg_out, inv_total):
    n_keys = keys.shape[0]
...
class APredictor:
    def __init__(self, num_nodes, num_rels, a_mode="energy", decay_a=1.0, ppr_beta=0.8, score_topk=0):
        self.num_nodes = int(num_nodes)
        self.num_rels = int(num_rels)
        self.a_mode = a_mode
        self.decay_a = float(decay_a)
        self.ppr_beta = float(ppr_beta)
        self.score_topk = max(0, int(score_topk))
    ...
    def _bucket_arrays(self, sr):
        ...
        if not bucket:
            ...
            if self.score_topk > 0 and len(values) > self.score_topk:
                keep = np.argpartition(values, -self.score_topk)[-self.score_topk :]
                keys = keys[keep]
                values = values[keep]
    ...
    def _update_rank(self, events):
        ...
        affected = {self._sr_key(s, r) for s, r in events_i64[:, :2]}
        dirty = set()
        for sr in affected:
            if sr not in self.V_sr:
                continue
            scale = self._rank_scale.get(sr, 1.0) * self.ppr_beta
            if scale < 1e-12:
                bucket = self.V_sr.get(sr)
                if bucket:
                    for obj in list(bucket.keys()):
                        bucket[obj] *= scale
                    self.V_sr_sum[sr] *= scale
                    dirty.add(sr)
                scale = 1.0
            self._rank_scale[sr] = scale

        for s, r, o in events_i64:
            sr = self._sr_key(s, r)
            bucket = self.V_sr.setdefault(sr, {})
            scale = self._rank_scale.get(sr, 1.0)
            raw_delta = 1.0 / max(scale, 1e-30)
            bucket[int(o)] = bucket.get(int(o), 0.0) + raw_delta
            self.V_sr_sum[sr] = self.V_sr_sum.get(sr, 0.0) + raw_delta
            dirty.add(sr)
        for sr in dirty:
            self._mark_dirty(sr)
...
def build_predictor(args, data):
    return APredictor(
        ...
        score_topk=getattr(args, "a_score_topk", 0),
    )
...
def get_out_dir(args):
    ...
    if int(getattr(args, "a_score_topk", 0)) > 0:
        params["a_score_topk"] = int(args.a_score_topk)
